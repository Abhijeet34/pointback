...
Do not add a `macos-*` or `windows-*` runner to a job that runs on `pull_request`, with the one exception the `cross-platform` job in `ci.yml` already is: merging the release pull request creates the tag, so that pull request alone calls the matrix, guarded on `startsWith(github.head_ref, 'release-please--')` and same-repository head, and every other one skips it.
