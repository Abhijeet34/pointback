...
- **The `cross-platform` matrix has never run.**
  Whether the whole suite passes on Windows is unknown: the product spawns a detached server, hands a URL to the platform's opener and canonicalises paths, and only that runner can say.
  Its first weekly run is the verification, and a red Windows leg on day one is information, not a regression.
